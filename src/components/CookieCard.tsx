...
interface CookieCardProps extends React.HTMLAttributes<HTMLDivElement> {
  cookie: SelectCookie;
}
...
export function CookieCard({ cookie, className, ...rest }: CookieCardProps) {
  ...
  return (
    <div {...rest} className={cn(className, 'flex items-center gap-2')}>
      <li id={cookie.name} className="group relative flex scroll-m-20 flex-col items-start rounded-lg bg-white">
